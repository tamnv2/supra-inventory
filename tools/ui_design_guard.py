...
checks = {
    ...
    "android_d114_picker_dense_split_xml": all(token in ANDROID_PICKER_XML for token in ['@+id/acSkuSearch', '@+id/btnReportShortage', '@+id/listMyReports', '@+id/panelShortage', '@+id/panelConfirmOrder', '@+id/tabShortage', '@+id/tabConfirmOrder', '@+id/etRelayPicklistSuffix', 'android:maxLength="20"', '@+id/relayCandidateList', 'android:layout_height="48dp"']),
    ...
    "android_d114_picker_split_behavior": all(token in ANDROID_PICKER for token in ["showOperationTab(confirm = false)", "showOperationTab(confirm = true)", "digits.length < 3", "renderRelayCandidates", "confirmAmbiguousPickList", "InputMethodManager.SHOW_IMPLICIT", "R.id.tabShortage", "R.id.tabConfirmOrder"]),
