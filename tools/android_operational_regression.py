...
def main() -> None:
    ...
    require(relay_agent_config, "AgentBuild = 41", "D120 Picker presence hotfix Agent build channel")
    ...
    require(relay, "TOTAL_WAIT_MS = 20_000L", "D117 terminal specialist fallback")
    ...
    require(firestore_leader, "PrimaryLeaseHeartbeatMs = 7000", "D117 PRIMARY lease heartbeat")
    ...
    require(firestore_leader, "FrozenRoleRefreshMs = 30000", "D117 early-start frozen role convergence")
    ...
    require(relay, 'Regex("^\\\\d{3,20}$")', "D114 Android one-term 3-20 digit suffix request")
    ...
    require(picker, 'Không tìm thấy PickList khớp đúng các số cuối', "D114 precise not-found result copy")
    ...
    require(exact_resolver, "suffix.Length < 3 || suffix.Length > 20", "D114 3-20 digit exact resolver")
    require(firestore_transport, "value.Length < 3 || value.Length > 20", "D114 transport suffix guard")
    require(firestore_transport, "picklist_suffix=redacted", "D105 generic redacted suffix audit")
    require(firestore_rules, "^[0-9]{3,20}$", "D114 Firestore suffix rule")
    require(relay, "candidatePicklists", "D114 PDA receives ambiguous PickList candidates")
    require(picker, "renderRelayCandidates", "D114 PDA renders ambiguous candidates")
    require(picker, "confirmAmbiguousPickList", "D114 PDA confirms exactly one selected candidate")
    require(picker, '.setNegativeButton("Huỷ", null)', "D114 cancel selected PickList without send")
    require(firestore_transport, "candidate_picklists", "D114 Agent returns bounded candidate list")
    require(relay_agent, 'Name = "ConfirmStatus"', "D114 Agent per-PickList result status")
    ...
    require(relay_agent, 'CheckAfterHoursSchedule', "D117 after-hours prompt loop")
    require(relay_agent, 'StartRelayBeforeSix', "D117 early-start relay control")
    require(relay_agent, 'ProbeWmsForTakeover', "D117 takeover-only WMS probe")
    require(firestore_leader, 'VerifyPrimaryBeforeMutation', "D117 generation mutation fence")
    require(firestore_leader, 'PublishScheduleDecision', "D117 shared schedule decision")
    require(firestore_leader, "ANY_AUTHENTICATED_AGENT", "D118 any Agent overtime decision")
    require(firestore_leader, "currentDocument.updateTime", "D118 schedule CAS")
    require(relay_agent, "Phát triển hệ thống · tamnv2 | Pick Pack 1291", "D118 Agent footer")
    require(firestore_leader, '"schedule_key"', "D117 schedule propagated in lease")
    require(firestore_leader, '"relay_override_until_ms"', "D117 relay override propagated in lease")
    require(relay_agent, "FINAL_20S_FENCE", "D117 final 20-second pre-WMS fence")
    require(relay_agent, "ROLE_GENERATION_FENCE", "D117 pre-WMS generation fence")
    require(relay_agent, 'TotalMinutes < 5', "D117 five-minute reminder")
    require(firestore_transport, '_businessEnabled()', "D117 scheduled relay business gate")
