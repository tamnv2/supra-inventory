...
def main() -> None:
    ...
    # D092: authenticated Firestore is the selected Office confirmation carrier.
    require(gradle, '"FIRESTORE_RELAY_COLLECTION_URL"', "D092 Firestore relay BuildConfig")
    require(relay, "ANDROID_CONFIRM_V1", "D092 Firestore confirmation request source")
    require(relay, 'header("Authorization", "Bearer " + session.idToken)', "D092 Picker Firebase bearer auth")
    require(relay, '.put("picker_uid", stringValue(identity.uid))', "D092 Picker Firebase identity")
    require(relay, '.put("picker_user_id", stringValue(session.userId))', "D092 Picker application identity")
    require(relay, "documentId", "D092 deterministic request document id")
    require(relay, "parseAck", "D092 Firestore ACK parser")
    require(relay, "cancelPending", "D092 conditional no-Agent cancellation")
    require(relay, "currentDocument.updateTime", "D092 conditional Picker cancellation")
    require(relay, "cleanupAfterAck", "D092 cleanup only after terminal ACK")
    forbid(relay, "FIREBASE_RTDB_URL", "D092 Android RTDB fallback")
    require(relay, "agentAdminUserId", "D092 ADMIN ACK identity")
    require(firestore_rules, "relay_poc_jobs", "D092 Firestore relay collection Rules")
    require(firestore_rules, "request.auth.token.app_base_role == 'ADMIN'", "D092 real ADMIN Firestore rule")
    require(firestore_rules, "resource.data.status == 'PENDING' || resource.data.status == 'ACK'", "D092 Picker cannot delete PROCESSING work")
    require(firestore_transport, "PollIntervalMs = 3000", "D092 bounded ACTIVE Agent polling")
    require(firestore_transport, '"PROCESSING"', "D092 job claim state")
    require(firestore_transport, "_canProcess", "D092 standby Agent does not poll jobs")
    ...
    require(relay, "PENDING_NOTICE_MS = 30_000L", "D095 non-terminal pending notice")
    require(relay, "Agent chưa nhận · vẫn tiếp tục kết nối", "D095 keep waiting after 30 seconds")
    require(relay, "lastStatus == \"PENDING\"", "D095 terminal pending check only after total wait")
    forbid(relay, "PENDING_CLAIM_WAIT_MS", "D095 no 30-second terminal cancellation")
    require(firestore_transport, "as IEnumerable", "D095 Firestore JSON-array parsing")
    forbid(firestore_transport, "DeserializeObject(raw) as ArrayList", "D095 broken runQuery ArrayList cast")
    forbid(firestore_transport, "docsObj as ArrayList", "D095 broken listDocuments ArrayList cast")
    require(relay_agent, "NetworkChange.NetworkAddressChanged", "D095 network-change proxy refresh")
    require(relay_agent, 'RefreshDefaultWindowsProxy("network-change")', "D095 dynamic Windows proxy refresh")
    require(picker, "api.createPickerReport(item.sku)", "D095 Báo hàng stays on Inventory API path")
    require(picker, "relayPocClient.sendProbe(suffix)", "D095 confirmation stays on dedicated Firestore relay path")
    forbid(relay, "/api/picker/reports", "D095 confirmation client must not use Báo hàng Worker mutation path")
    ...
    require(firestore_leader, "FailoverAfterMs = 10000", "D092 Agent failover threshold")
    require(firestore_leader, "currentDocument.updateTime", "D092 conditional leader renewal")
    require(firestore_rate, "StrikeWindowMs = 60L * 1000L", "D092 Firestore strike window")
    require(firestore_rate, "return 5;", "D092 first lock")
    require(firestore_rate, "return 30;", "D092 second lock")
    require(firestore_rate, "return 60;", "D092 third lock")
