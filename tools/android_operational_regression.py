...
def main() -> None:
    ...
    firestore_transport = read("relay-agent/FirestoreRelayTestTransport.cs")
    ...
    # D091: authenticated Firestore field carrier replaces RTDB only for the current physical PDA↔Agent proof.
    require(gradle, '"FIRESTORE_RELAY_COLLECTION_URL"', "D091 Firestore relay BuildConfig")
    require(relay, "ANDROID_D091", "D091 Firestore request source")
    require(relay, 'header("Authorization", "Bearer " + session.idToken)', "D091 Picker Firebase bearer auth")
    require(relay, '.put("picker_uid", stringValue(identity.uid))', "D091 Picker Firebase identity")
    require(relay, '.put("picker_user_id", stringValue(session.userId))', "D091 Picker application identity")
    require(relay, "documentId", "D091 deterministic request document id")
    require(relay, "parseAck", "D091 Firestore ACK parser")
    require(relay, "cleanup(documentUrl, session.idToken)", "D091 Picker-owned cleanup")
    forbid(relay, "FIREBASE_RTDB_URL", "D091 Android RTDB field fallback")
    require(relay, "agentAdminUserId", "D091 ADMIN ACK identity")
    require(firestore_rules, "relay_poc_jobs", "D091 Firestore relay collection Rules")
    require(firestore_rules, "request.auth.token.app_base_role == 'ADMIN'", "D091 real ADMIN Firestore rule")
    require(firestore_rules, "resource.data.picker_uid == request.auth.uid", "D091 Picker-owned Firestore get/delete")
    require(firestore_transport, "PollIntervalMs = 3000", "D091 bounded Agent polling")
    require(firestore_transport, '"TRANSPORT_ONLY"', "D091 transport-only ACK")
    ...
    require(relay_agent_config, "AgentBuild = 16", "D091 Agent build channel")
