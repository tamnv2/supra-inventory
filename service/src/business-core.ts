import { planAutoSkipForNewReport, scheduleNextOperationalAlarm, skipCorrectionDeadlineForBatch } from "./sla-automation";
  ...
  const result = state.storage.transactionSync(() => {
    ...
    if (Number(activeRemaining?.count || 0) === 0) {
      ...
      if (Number(timedOut?.count || 0) > 0) {
        const correctionDeadline = skipCorrectionDeadlineForBatch(state, ticket.batch_id);
        state.storage.sql.exec(
          `UPDATE report_tickets
...
async function resolveBatch(state: DurableObjectState, request: Request): Promise<BusinessResult> {
  ...
  const result = state.storage.transactionSync(() => {
    ...
    const at = nowIso();
    const correctionDeadline = resolution === "SKIP_ALLOWED" ? skipCorrectionDeadlineForBatch(state, batchId) : null;
