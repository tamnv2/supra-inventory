...
import { auth, firebaseMissing, firebaseReady, usernameToEmail } from "./firebase";
import { getHrSource, getMyProfile, saveHrSource, type AppProfile } from "./api";
...
function render(): void {
  if (!firebaseReady || !auth) {
    app.innerHTML = `
      <main class="shell">
        <section class="card">
          <p class="eyebrow">SUPRA Inventory — Beta</p>
          <h1>Thiếu cấu hình Firebase Web</h1>
          <p>Source Web đã sẵn sàng nhưng chưa thể đăng nhập cho tới khi cấu hình public Firebase được nạp.</p>
          <pre>${escapeHtml(firebaseMissing.join(", "))}</pre>
        </section>
      </main>`;
    return;
  }
      ...
      <main class="shell">
        <section class="card login-card">
          ...
          <form id="login-form" class="stack">
            <label>Tên đăng nhập<input name="username" autocomplete="username" value="root" required /></label>
            <label>Mật khẩu<input name="password" type="password" autocomplete="current-password" required /></label>
            <button ${busy ? "disabled" : ""}>${busy ? "Đang đăng nhập..." : "Đăng nhập"}</button>
          </form>
  ...
  app.innerHTML = `
    <main class="shell wide">
      <header class="topbar card">
        <div><p class="eyebrow">SUPRA Inventory — Beta</p><h1>${escapeHtml(profile?.display_name || "Đang tải hồ sơ...")}</h1></div>
        <div class="actions"><span class="badge">${escapeHtml(profile?.role || "AUTH")}</span><button id="logout" class="secondary">Đăng xuất</button></div>
      </header>
      ${message ? `<p class="message card">${escapeHtml(message)}</p>` : ""}
      <section class="grid">
        <article class="card">
          <h2>Đổi mật khẩu</h2>
        ...
        <article class="card">
          <h2>Nguồn nhân sự</h2>
          <p>Nhập link Google Sheet và tên tab chính xác. Backend chỉ lưu sau khi kiểm tra quyền đọc + cột MNV/Họ tên.</p>
          <form id="hr-form" class="stack">
            <label>Google Sheet URL<input name="sheetUrl" type="url" required /></label>
            <label>Tên tab<input name="tabName" value="Nhân sự" required /></label>
            <button>Xác nhận & cập nhật</button>
          </form>
          <button id="load-hr" class="secondary">Đọc cấu hình hiện tại</button>
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      try {
        ...
        message = error instanceof Error ? error.message : "Không tải được hồ sơ ứng dụng.";
      }
    }
    render();
  });
