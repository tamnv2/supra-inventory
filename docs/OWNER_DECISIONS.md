...
| D041 | ACTIVE | Android/PDA must implement the approved narrow-screen layout, labels, footer and mandatory update gate directly in the primary activity/view source. Do not use an Application-level/post-layout tree rewriter to hide, rename or restyle business UI after rendering; this avoids layout races, repeated traversal overhead and device-dependent alignment. |
| D042 | ACTIVE | Android/PDA Picker and Reporter use the Owner-approved operational layout while retaining the Practical Balanced visual system. Shared header shows `BÁO HÀNG 1291`, user identity, `Log`, `Thoát`, role and compact Beta version. Picker uses one horizontal SKU-entry + `BÁO HẾT HÀNG` action followed by `Lịch sử báo hàng hôm nay` with status-colored cards. Reporter uses four state filters `Đang xử lý`, `Đã có hàng`, `Đã cho skip`, `Picker thu hồi`; pending cards show SKU/product, first-report context/affected count and two equal actions `CÓ HÀNG` / `CHO SKIP HÀNG`. Existing server-authoritative dedupe, 60-second withdrawal, Reporter priority, resolution and 5-minute correction semantics remain unchanged. The Reporter withdrawn view must represent real closed batches caused by Picker withdrawal rather than a decorative empty tab. |
