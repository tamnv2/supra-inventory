...
4. **Role presentation and capability.** Existing internal `ADMIN` remains the identifier and is displayed as **Quản trị Invent**. New internal role `PICKPACK_ADMIN` is displayed as **Quản trị Pick Pack**. It may manage Picker personnel, SKU manual/import workflows, read shortage/reporting/history/export and the bounded Picker-contact capability; it cannot resolve Có hàng/Cho phép skip, change SLA/auto-skip/processing-time policy, reset system, manage Invent/Root authority or inherit WMS confirmation implicitly.
...
11. **Background/critical Android alerts.** Existing shortage business state remains InventoryCore authority. High-priority FCM is the background wake path; no all-day socket/heartbeat is added. The 05:00–23:00 operational alert window uses server-authoritative Asia/Ho_Chi_Minh time, with explicit overtime extension authority. Shortage critical alerts may use the OS overlay permission when granted and must fall back to the accepted notification path when unavailable. Xác nhận đơn keeps its accepted in-app result model.
