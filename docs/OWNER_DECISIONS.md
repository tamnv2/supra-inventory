...
2. Agent must remain independent from Inventory Cloudflare Worker on the Office network. D100 therefore provisions one deterministic ADMIN-only Firebase Agent alias per real ADMIN username, synchronized with the same password/status and carrying the same `app_user_id`/ADMIN claims. ROOT/REPORTER/PICKER never receive an Agent alias.
3. Primary Firebase identity remains the Web/App/recovery identity. Changing an ADMIN password or status must synchronize both primary and Agent alias. Registered recovery email changes only the primary identity; Agent alias address is synthetic/internal.
4. Beta health must fail until all ACTIVE ADMIN users have both primary Firebase credentials and Agent aliases ready. Schema target advances to 10 only for additive Agent-alias readiness state.
...
10. Picker/Reporter/Admin account reset deletes corresponding InventoryCore users and Firebase Auth identities. ADMIN reset also deletes its Agent alias. Google Sheet HR source rows are never deleted.
...
16. Stable remains **OWNER-GATED** and untouched.
